...
import { fetchAllDashboardData, fetchPortNews, derivePortSignals } from "./services/api_integration";
...
export default function Terminal() {
  ...
  const [wtiH, setWtiH] = useState(() => genWTI());
  ...
  // Live dashboard data — refreshes every 5 min
  useEffect(() => {
    const load = async () => {
      const data = await fetchAllDashboardData();
      setLiveData(data);
      if (data?.wtiHistory?.length) {
        const c = data.cushing?.current ?? OG.cushing;
        setWtiH(data.wtiHistory.map((r, i) => ({ week: i, wti: r.wti, cushing: c })));
      }
      if (data?.sced?.length) setSCED(data.sced.slice(-18));
      if (data?.regime?.regime) setRegime(data.regime.regime);
      if (data?.macroCtx) setLivePorts(p => derivePortSignals(p, data.macroCtx));
    };
    load();
  ...
  // Port news via GDELT — fetches once on mount, refreshes every 30 min
  useEffect(() => {
    const load = async () => {
      const news = await fetchPortNews();
      if (Object.keys(news).length)
        setLivePorts(prev => prev.map(p => news[p.id] ? { ...p, intel: news[p.id].intel } : p));
    };
    load();
    const iv = setInterval(load, 1_800_000);
    return () => clearInterval(iv);
  }, []);
  ...
  const score = useMemo(() => {
    if (liveData?.wxScore) {
      const LBL = { thermal: "Thermal Load", solar: "Solar Avail", wind: "Wind Gen" };
      const CLR = { thermal: C.red, solar: C.amber, wind: C.blue };
      return { composite: liveData.wxScore.composite, details: liveData.wxScore.details.map(s => ({ ...s, label: LBL[s.id] ?? s.id, color: CLR[s.id] ?? C.tx })) };
    }
    return calcScore(horizon);
  }, [horizon, liveData]);
  const aPort = sPort || hPort;
  const lmp = sced[sced.length - 1]?.lmp;
  const lC = lmp > 300 ? C.red : lmp > 80 ? C.amber : lmp < 0 ? C.blue : C.green;
  const hW = useMemo(() => ({
    temp:  liveData?.weather?.tempF           ?? 94,
    solar: liveData?.weather?.solarIrradiance ?? 920,
    cloud: liveData?.weather?.cloudPct        ?? 12,
    wx:    liveData?.wxScore?.composite       ?? 0.42,
  }), [liveData]);
  const og = useMemo(() => ({
    ...OG,
    wti:        liveData?.prices?.wti      ?? OG.wti,
    wtiChg:     liveData?.prices?.wtiChg   ?? OG.wtiChg,
    brent:      liveData?.prices?.brent    ?? OG.brent,
    brentChg:   liveData?.prices?.brentChg ?? OG.brentChg,
    hh:         liveData?.prices?.hh       ?? OG.hh,
    hhChg:      liveData?.prices?.hhChg    ?? OG.hhChg,
    crack321:   liveData?.crackSpread?.crack321 ?? OG.crack321,
    crackChg:   liveData?.crackSpread?.crack321 != null
                  ? +(liveData.crackSpread.crack321 - OG.crack321).toFixed(2)
                  : OG.crackChg,
    cushing:    liveData?.cushing?.current ?? OG.cushing,
    cushingChg: liveData?.cushing?.change  ?? OG.cushingChg,
    refinery:   liveData?.refinery?.length
                  ? liveData.refinery.map(rv => ({ ...OG.refinery.find(o => o.region === rv.region) ?? {}, ...rv }))
                  : OG.refinery,
  }), [liveData]);
  const macroCtx = liveData?.macroCtx;
    ...
    <div style={{ fontFamily: sans, background: C.bg, color: C.tx, minHeight: "100vh", fontSize: 14 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 20px", background: C.panel, borderBottom: `1px solid ${C.border}`, opacity: entered ? 1 : 0, transition: "opacity 0.6s" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16, fontFamily: mono, fontSize: 11 }}>
          ...
          {liveData?.macro?.dxy && <span style={{ color: C.td }}>DXY <span style={{ color: C.white }}>{liveData.macro.dxy.toFixed(1)}</span></span>}
              ...
              <Panel title="CUSHING STORAGE">
                <div style={{ padding: 16 }}>
                  <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
                    ...
                    <span style={{ fontSize: 11, fontFamily: mono, color: og.cushingChg >= 0 ? C.green : C.red }}>{og.cushingChg >= 0 ? "+" : ""}{og.cushingChg.toFixed(1)}</span>
                  </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 14 }}>
              <Panel title="GLOBAL OIL PORT NETWORK" rightLabel="DRAG TO ROTATE">
                <div style={{ background: "#080c14", padding: 8 }}>
                  <PortsGlobe countries={countries} ports={livePorts} activePort={aPort} hPort={hPort} setHP={setHP} sPort={sPort} setSP={setSP} rot={pRot} setRot={setPRot} />
                </div>
                <Legend ports={livePorts} />
              </Panel>
              <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
                <div style={{ padding: "8px 14px", borderBottom: `1px solid ${C.border}`, background: C.panel, border: `1px solid ${C.border}`, borderRadius: "3px 3px 0 0" }}>
                  <span style={{ fontSize: 10, fontFamily: mono, letterSpacing: 2, color: C.td }}>PORT ROSTER · {livePorts.length} TRACKED</span>
                </div>
                  ...
                  {livePorts.map((p, i) => {
                    ...
                    return (
                      <div key={p.id} onClick={() => setSP(sPort?.id === p.id ? null : p)} onMouseEnter={() => setHP(p)} onMouseLeave={() => setHP(null)}
                        style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 14px", cursor: "pointer", borderBottom: i < livePorts.length - 1 ? `1px solid ${C.border}` : "none", borderLeft: isA ? `3px solid ${p.signalColor}` : "3px solid transparent", background: isA ? `${p.signalColor}08` : "transparent", transition: "all 0.2s" }}>
                        <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 14 }}>
              <Panel title="SOLAR IRRADIANCE & TEMPERATURE" rightLabel="DRAG TO ROTATE">
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 14px", borderTop: `1px solid ${C.border}`, fontSize: 9, fontFamily: mono }}>
                  ...
                  <div style={{ flex: 1 }} />
                  <span style={{ color: C.td }}>{livePorts.filter(p => p.signal === "BULLISH").length} BULLISH · {livePorts.filter(p => p.signal === "BEARISH").length} BEARISH</span>
                </div>
              </Panel>
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                ...
                {macroCtx && (() => {
                  const ps  = macroCtx.policy_state;
                  const sp  = ps?.sharpe_projection;
                  const e7v = ps?.e7 ?? macroCtx.jacobian_signal_vector?.s7_dxy_wti_r ?? 0;
                  const e7Status = ps?.e7_status ?? (Math.abs(e7v) >= 0.25 ? "HARD_STOP" : Math.abs(e7v) >= 0.08 ? "YELLOW" : "CLEAR");
                  const e7Color  = e7Status === "HARD_STOP" ? C.red : e7Status === "YELLOW" ? C.amber : C.green ?? "#4caf50";
                  const firing   = ps?.firing ?? false;
                  const scale    = ps?.position_scale ?? 0;
                  const rebalDays= ps?.rebalance_days ?? 10;

                  // Sharpe tooltip content (shown on hover of regime label)
                  const sharpeTooltip = sp ? [
                    `Policy: ${firing ? "ACTIVE" : "HOLD"}`,
                    `Scale: ${(scale * 100).toFixed(0)}% of max position`,
                    `Clock: ${rebalDays}-day rebalance`,
                    ``,
                    `Forward Sharpe: ${sp.expected_sharpe?.toFixed(3) ?? "—"}`,
                    `90% CI: [${sp.sharpe_ci90?.[0]?.toFixed(2)}, ${sp.sharpe_ci90?.[1]?.toFixed(2)}]`,
                    `Basis: ${sp.basis ?? "—"}`,
                    ``,
                    `e7 (DXY×WTI 90d ρ): ${e7v >= 0 ? "+" : ""}${e7v.toFixed(4)}  [${e7Status}]`,
                    ps?.summary ?? "",
                  ].filter(Boolean).join("\n") : "Policy data unavailable";

                  return (
                    <Panel title="MACRO CONTEXT" rightLabel="ORBIT ENGINE">
                      <div style={{ padding: 14 }}>

                        {/* Regime label + Sharpe inline */}
                        <div>
                          <div style={{ fontSize: 12, fontFamily: mono, color: C.amber, fontWeight: 600 }}>
                            {macroCtx.macro_regime?.replace(/_/g, " ").toUpperCase()}
                          </div>
                          <div style={{ fontSize: 9, fontFamily: mono, color: C.td, marginTop: 2 }}>
                            {(macroCtx.regime_confidence * 100).toFixed(0)}% confidence
                            {sp && sp.expected_sharpe > 0
                              ? <> · <span style={{ color: C.amber }}>Sharpe {sp.expected_sharpe.toFixed(2)}</span> <span style={{ color: C.tf }}>CI [{sp.sharpe_ci90?.[0]?.toFixed(2)}, {sp.sharpe_ci90?.[1]?.toFixed(2)}]</span></>
                              : <> · <span style={{ color: C.tf }}>cycle score {macroCtx.composite_score?.toFixed(3)}</span></>
                            }
                          </div>
                        </div>

                        {/* Policy firing indicator */}
                        {ps && (
                          <div style={{ marginTop: 6, fontSize: 9, fontFamily: mono }}>
                            <span style={{
                              color: firing ? (C.green ?? "#4caf50") : C.red,
                              fontWeight: 600, letterSpacing: 0.5,
                            }}>
                              {firing ? `ACTIVE  ${rebalDays}d  ${(scale*100).toFixed(0)}% sz` : "HOLD"}
                            </span>
                            {sp && firing && (
                              <span style={{ color: C.td, marginLeft: 6 }}>
                                SR {sp.expected_sharpe?.toFixed(2)}
                              </span>
                            )}
                          </div>
                        )}

                        {macroCtx.live_ercot_context && (
                          <div style={{ marginTop: 8, fontSize: 9, fontFamily: mono, color: C.tm, lineHeight: 1.9 }}>
                            <div>Solar {(macroCtx.live_ercot_context.solar_pct * 100).toFixed(0)}% · Wind {(macroCtx.live_ercot_context.wind_pct * 100).toFixed(0)}% · Gas {(macroCtx.live_ercot_context.gas_pct * 100).toFixed(0)}%</div>
                            <div>
                              ORDC <span style={{ color: macroCtx.live_ercot_context.ordc_adder > 5 ? C.amber : C.tm }}>
                                ${macroCtx.live_ercot_context.ordc_adder?.toFixed(2)}/MWh
                              </span> · Lambda ${macroCtx.live_ercot_context.system_lambda?.toFixed(0)}/MWh
                            </div>

                            {/* s4 / s7 with hover tooltips */}
                            <div style={{ color: C.tf }}>
                              <span>s4={macroCtx.jacobian_signal_vector?.s4_asdc_scarcity_proxy?.toFixed(3)}</span>
                              {" · "}
                              <span style={{ color: e7Color }}>
                                e7={e7v >= 0 ? "+" : ""}{e7v.toFixed(3)} [{e7Status}]
                              </span>
                            </div>
                          </div>
                        )}
